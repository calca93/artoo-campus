   ...
   this.updateAvailability = function (item, amount){
      items.forEach(function(singleItem){
         if(singleItem.code === item.code)
            singleItem.availability = ((singleItem.availability + amount) < 0) ? 0 : singleItem.availability += amount;
      });
   };

   //remove
   this.remove = function (item){
       items = items.filter(singleItem => singleItem.code !== item.code);//RITORNA LA LISTA CON OGGETTI CHE VERIFICANO LA CONDIZIONE
   };
